document.addEventListener("DOMContentLoaded", function () {
  ...
  function getMouthSrc(actor, size) {
    ...
    if (actor === "toyworker") {
      const toyWorkerFiles = {
        closed: "/static/images/toy-store-worker-mouth-closed.png",
        small: "/static/images/toy-store-worker-mouth-small.png",
        medium: "/static/images/toy-store-worker-mouth-medium.png",
        wide: "/static/images/toy-store-worker-mouth-wide.png"
      };
    ...
    round.bins.forEach(bin => {
      ...
      binButton.innerHTML = `
        ...
        <div class="bin-items" id="binItems-${escapeHtml(bin.id)}">
          <span class="empty-shelf-text">Tap toys here</span>
        </div>
      `;
  ...
  async function sortToyIntoBin(toy, bin) {
    ...
    if (binItems) {
      const emptyText = binItems.querySelector(".empty-shelf-text");
