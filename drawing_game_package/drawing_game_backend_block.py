...
def drawing_game_complete():
    ...
    try:
        ...
        # Unlock the next active activity in the same journey order.
        # If drawing_game is inserted immediately before book_guessing_game,
        # this will send the child to Book Guessing Game.
        cursor.execute("""
            SELECT activity_id
            FROM activity
            WHERE is_active = 1
