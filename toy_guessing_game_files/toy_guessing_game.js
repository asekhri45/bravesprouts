document.addEventListener("DOMContentLoaded", function () {
  ...
  async function playThinkingFillerLine() {
    ...
    try {
      ...
      const response = await fetch(`/api/toy-guessing-game/thinking-audio?avoid=${avoid}`, {
        method: "GET",
  ...
  async function requestToyWorkerMessage(eventType, childResponse = "") {
    ...
    try {
      const response = await fetch("/api/toy-guessing-game/message", {
        method: "POST",
        headers: {
  ...
  async function sendAudioToTranscribe(audioBlob) {
    ...
    try {
      ...
      const response = await fetch("/api/toy-guessing-game/transcribe", {
        method: "POST",
        credentials: "same-origin",
        body: formData
      });
