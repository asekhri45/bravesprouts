...
function getMicrophoneErrorType(error) {
  ...
  function markPermissionButtonReady(button, readyText) {
    if (!button) return;
  ...
  function markPermissionButtonNotReady(button, text) {
    if (!button) return;
  ...
  function getPermissionsHelper() {
    return window.BraveSproutPermissions || null;
  }
  ...
  function refreshVisibleRequirements() {
    ...
    if (setupState.audioReady) {
      markPermissionButtonReady(
        enableSetupAudioBtn,
        "Audio Enabled ✓"
      );
    } else {
      markPermissionButtonNotReady(
        enableSetupAudioBtn,
        "Enable Audio"
      );
    }
  ...
  if (startActivityBtn) {
    startActivityBtn.addEventListener(
      "click",
  ...
  if (enableSetupAudioBtn) {
    enableSetupAudioBtn.addEventListener(
      "click",
      async function () {
        const permissions = getPermissionsHelper();
        ...
        if (
          !permissions ||
          typeof permissions.unlockAudio !== "function"
        ) {
          setSetupMessage(
            "Audio setup is unavailable. Please refresh and try again.",
            "error"
          );
          return;
        }
        ...
        const originalText =
          enableSetupAudioBtn.textContent;

        enableSetupAudioBtn.disabled = true;
        enableSetupAudioBtn.textContent =
          "Enabling Audio...";
        ...
        try {
          ...
          if (!successful) {
            throw new Error(
              result?.message ||
              "Your browser could not enable audio."
            );
          }
          ...
          markPermissionButtonNotReady(
            enableSetupAudioBtn,
            originalText || "Enable Audio"
          );

          setSetupMessage(
            error.message ||
            "Audio could not be enabled. Please try again.",
            "error"
          );
        }
      }
  ...
  if (enableSetupMicrophoneBtn) {
    enableSetupMicrophoneBtn.addEventListener(
      "click",
      async function () {
        const permissions = getPermissionsHelper();
        ...
        if (
          ...
          setMicrophoneError(
            "Microphone setup is unavailable. Please refresh and try again."
          );
          return;
        }

        const originalText =
          enableSetupMicrophoneBtn.textContent;

        enableSetupMicrophoneBtn.disabled = true;
        enableSetupMicrophoneBtn.textContent =
          "Requesting Access...";
        ...
        try {
          ...
          localStorage.setItem(
            "bravesprouts_microphone_enabled",
            "true"
          );
          ...
          markPermissionButtonNotReady(
            enableSetupMicrophoneBtn,
            originalText || "Enable Microphone"
          );
