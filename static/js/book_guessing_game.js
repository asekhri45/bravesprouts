document.addEventListener("DOMContentLoaded", function () {
  ...
  async function requestCharacterMessage(eventType, childResponse = "") {
    try {
      ...
      console.log("📚 Librarian response:", data);

      if (!data.success) {
        console.error(data.error || "Librarian response failed");
        return;
      }
      ...
      console.error("Book Guessing Game request error:", error);
    }
  }
  ...
  function playCharacterAudio(audioSrc, shouldListenAfter = true) {
    ...
    characterAudio.playbackRate = 1.03;
  ...
  function startMouthAnimation() {
    ...
    function setMouth(state, scaleX, scaleY) {
      if (currentMouth !== state) {
        mouth.src = `/static/images/librarian_mouth-${state}.png`;
        currentMouth = state;
      }
  ...
  function stopMouthAnimation() {
    ...
    if (mouth) {
      mouth.src = "/static/images/librarian_mouth-closed.png";
  ...
  function startGameAfterCall() {
    ...
    bookStage.classList.remove("call-hidden");
