document.addEventListener("DOMContentLoaded", function () {
  ...
  function dlog(...args) { if (window.APP_DEBUG) console.log(`[guessing_game_2:${activityId}]`, ...args); }
  ...
  async function playThinkingFillerLine() {
    ...
    try {
      ...
      const response = await fetch(`/api/guessing-game-2/thinking-audio?avoid=${avoid}`, {
        method: "GET",
  ...
  async function requestToyWorkerMessage(eventType, childResponse = "") {
    ...
    try {
      const response = await fetch("/api/guessing-game-2/message", {
        method: "POST",
        headers: {
      ...
      const data = await response.json();
      console.log("⭐ Guessing Game Toy worker response:", data);
  ...
  function startLiveSpeechRecognition() {
    ...
    recognitionHardCapTimer = setTimeout(function () {
      if (!recognitionActive || roundResolved || waitingForToyWorker || sessionDone || !gameActive) return;
  ...
  function submitLiveTranscript(cleanedTranscript) {
    if (waitingForToyWorker || sessionDone || !gameActive || roundResolved) return;
    roundResolved = true;
  ...
  async function sendAudioToTranscribe(audioBlob) {
    ...
    try {
      ...
      const response = await fetch("/api/guessing-game-2/transcribe", {
        method: "POST",
        credentials: "same-origin",
        body: formData
      });
  ...
  function playCharacterAudio(audioSrc, expectsResponse = true, onEnded = null) {
    ...
    characterAudio.addEventListener("error", function () {
      console.error("Toy worker audio error");
      stopMouthAnimation();
