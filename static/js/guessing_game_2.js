document.addEventListener("DOMContentLoaded", function () {
  ...
  const guess2Stage = document.getElementById("guess2Stage");
  ...
  async function requestCharacterMessage(eventType, childResponse = "") {
    ...
    try {
      ...
      console.log("🧸 Guessing Game 2 response:", data);

      if (!data.success) {
        console.error(data.error || "Character response failed");
        return;
      }
      ...
      console.error("Guessing Game 2 request error:", error);
    }
  }
  ...
  async function sendAudioToTranscribe(audioBlob) {
    ...
    try {
      ...
      requestCharacterMessage("child_response", transcript);
  ...
  function startGameAfterCall() {
    ...
    guess2Stage.classList.remove("call-hidden");
