document.addEventListener("DOMContentLoaded", function () {
  ...
  function getTile(actor) {
    const introVisible = introScreen && !introScreen.classList.contains("hidden") && introScreen.style.display !== "none";
  ...
  function isYesText(text) {
    return /\b(yes|yeah|yep|sure|okay|ok|again|more|another|one more)\b/i.test(text);
  }
  ...
  async function handleSpeech(transcript, question) {
    ...
    if (question.source === "librarian-direct") {
      await queueSpeak("librarian", pickLine([
  ...
  function handleNoSpeech(question) {
    ...
    // For normal drawing small-talk, silence is okay. No correction line.
  }

  async function handlePlayAgainResponse(transcript) {
    if (isNoText(transcript) && state.roundNumber <= 2) {
      await queueSpeak("star", "Okay. We can stop drawing for now.");
  ...
  async function handleFinalPlayAgainResponse(transcript) {
    if (isNoText(transcript)) {
      await queueSpeak("librarian", "Okay. We can try the next game.");
  ...
  async function beginRound() {
    ...
    if (state.roundNumber === 1) {
      await queueSpeak("star", pickLine(prompt.starStarter), {
        expectsResponse: true,
        askType: "one_word",
        source: "star",
        responseSeconds: 5.2
  ...
  async function finishRound() {
    if (!hasDrawnThisRound || state.gameCompleted) return;

    doneDrawingBtn.disabled = true;
    state.roundsCompleted += 1;

    if (state.finalRoundStarted) {
      await queueSpeak("librarian", `Nice drawing, ${childName}. Let's try the next game.`);
      await completeAndGoNext();
      return;
    }

    if (state.roundNumber === 1) {
      await queueSpeak("librarian", `Nice drawing, ${childName}.`);
    } else if (state.roundNumber === 2) {
      await queueSpeak("librarian", `That looks good, ${childName}.`);
    } else if (state.roundNumber === 3) {
      await queueSpeak("librarian", "I liked drawing that one with you.");
    } else {
      await queueSpeak("librarian", `Thanks for telling me your idea, ${childName}.`);
    }
  ...
  async function askPlayAgain() {
    await queueSpeak("librarian", "Do you two want to play again?");
    await queueSpeak("star", `What do you think, ${childName}?`, {
      expectsResponse: true,
      askType: "open",
  ...
  async function offerFinalPlayAgain() {
    ...
    await queueSpeak("librarian", `Do you want to draw one more before we play a different game, ${childName}?`, {
      expectsResponse: true,
      askType: "open",
      source: "librarian-direct",
